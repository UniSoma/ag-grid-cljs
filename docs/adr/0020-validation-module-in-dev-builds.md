...
- **No effect on `update-grid!`'s initial-only warning** (agd-01kymx8m23sj). *Corrected 2026-07-28 — this bullet originally claimed the registration dissolved that ticket's blocker.* It does not. AG Grid's `_warn(22)` is emitted only from `warnOnInitialPropertyUpdate`, whose sole caller is `updateGridOptions` — the body of `setGridOption`, which the `:initial-only` branch exists to skip. Registration changes nothing on a path the key never travels, so the wrapper's warning is not redundant and stays. See the reclassified entry in ADR 0017's appendix.
...
- **Register it behind `enable-dev-validations!`** — rejected: uniformity for its own sake, again. The gate contains registry drift; this module has no registry of ours to drift.
...
- ADR 0008 — options diffing (the initial-only warning this was wrongly thought to unblock)
