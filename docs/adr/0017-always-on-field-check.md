...
7. **Dot notation gated on `Column.isFieldContainsDots()`, first segment only.** The grid option `suppressFieldDotNotation` turns dots into literal characters in a key name; `Column.initDotNotation` already resolves this once at column-build time, so we ask rather than re-derive and cannot drift from AG Grid. Only the first segment is checked: nested objects are legitimately sparse (`{address: null}` on one row, populated on another), so walking deeper would trade a rare catch for a common false positive.
   ...
   > **Scoped to this check by ADR 0019 §9 (2026-07-29, agd-01kyqmb9ssq7).** The first-segment rule, and the Considered-options rejection of "walking the row's dotted path" below, are about a **presence** test — which is what makes a missing hop indistinguishable from sparse data, and therefore a false positive. The ref-data check needs the **value**, and there a missing hop yields a non-string, which that check treats as silence. So it walks the whole path, in `impl.validate` beside this one, without contradicting either clause: the rejection stands for presence tests and does not generalize to value reads.
...
- §2's events are **async-dispatched** (neither is in `ALWAYS_SYNC_GLOBAL_EVENTS`), and AG Grid's queue flush does not consult listener removal — so an event dispatched in the same macrotask as `api.destroy()` runs the check against the dead grid, where any api call except `isDestroyed` prints the ungated "grid has been destroyed" warning. `run-live-check!` therefore opens with an `isDestroyed` guard, covering every live-grid check; a browser test pins the window (2026-08-11, agd-01kzq9x25fez).
...
- **Initial-only warning** in `update-grid!` (ADR 0008). *Reclassified 2026-07-28 — the original audit filed this under "redundant" and was wrong; see the correction below.* AG Grid does have the text — `_warn(22)`, "`{key}` is an initial property and cannot be updated" (56791) — but it is emitted from `ValidationService.warnOnInitialPropertyUpdate` (57488), whose sole caller is `GridOptionsService.updateGridOptions` (27157), i.e. the body of `setGridOption`. The `:initial-only` branch never calls `setGridOption` — skipping it *is* the branch — so upstream never sees the key and never warns. The *skip* likewise duplicates nothing: AG Grid warns and then, if the value differs from the current one (27160), still writes `gridOptions[key] = value` (27161) and dispatches the property event, leaving state carrying a value no component reads. Both halves stay. The sibling `:unclassified` branch has no upstream analogue either — it is about our registry alone. The upstream half of that argument is pinned by one browser test (`ag-grid-cljs.browser.initial-only-test`, ADR 0015 §1: it is a claim about AG Grid's runtime, not our contract) — an `:initial? true` key through `update-grid!` draws no `_warn(22)`, the same key pushed straight at the grid api does.

  **Correction.** The audit's claim that `_warn(22)` "fires when `source === "api"` — exactly our `setGridOption` path" is struck. It is our path for the `:updatable` and `:unclassified` branches, which by definition are not initial-only keys; on the one branch where the warning would matter we make no call. The follow-up caveat — that removal waited on ValidationModule registration becoming unconditional — is struck with it: gating was never the obstacle, so ADR 0020 did not unblock anything here (agd-01kymx8m23sj).
...
- **Deprecation warnings** (ADR 0007 §5). Redundant *and strictly narrower*. `processOptions` (57570) emits `As of v{version}, {name} is deprecated. {message}` from `GRID_OPTION_DEPRECATIONS` (26389) and `COLUMN_DEFINITION_DEPRECATIONS` (25490): **106 keys against our registry's 32**, same replacement text from the same upstream source. Diffing camel names, the only key ours flags that AG Grid's runtime tables miss is `reactiveCustomComponents` — a framework-adapter option with no meaning for a vanilla-core wrapper (ADR 0012). The gap is explained by origin: our registry records `@deprecated` tsdoc tags found during codegen, AG Grid's tables are hand-maintained and wider.
  ...
  **Outcome:** removed. The gates were independent opt-ins, not one condition, so the deletion required lining them up first — ADR 0020 has `create-grid!` register ValidationModule in `goog.DEBUG` builds and deletes the wrapper branch against that. Reachability is not in doubt here, and was checked rather than assumed: `validateGridOptions` → `validation?.processGridOptions` runs from `GridOptionsService.postConstruct` (27098) at creation and again at the end of every `updateGridOptions` (27172), over the whole options object — paths the wrapper does take.
...
Verify it at the *call site*, not the text. The initial-only reclassification above is what this sentence is for: the message existed upstream, so the entry read as redundant, but the only caller sits behind a call the wrapper deliberately does not make. Finding the string is the start of the check — trace who calls it, and whether our code path reaches them.
