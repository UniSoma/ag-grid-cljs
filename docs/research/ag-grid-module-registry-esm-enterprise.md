# AG Grid Module Registry, ESM Packaging, and Enterprise Licensing Surface

> What the wrapper must expose for AG Grid v34's module system and Enterprise licensing:
> how `ModuleRegistry.registerModules` works across the `ag-grid-community` /
> `ag-grid-enterprise` ESM packages, module→feature mapping and bundle-size cost,
> `LicenseManager.setLicenseKey` application, interaction with shadow-cljs tree-shaking,
> and whether the wrapper should own module registration.
>
> Origin: knot research ticket `agd-01ky0eck3myz`, resolved 2026-07-20.
>
> **Caveat:** findings are version-pinned to the AG Grid versions studied (v33/v34,
> package sizes measured at 34.3.1). Version-sensitive claims — module names, package
> layout, bundle sizes, licensing behavior — should be re-checked against the actual
> AG Grid version at implementation time.
