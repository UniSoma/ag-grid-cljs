# Options-Surface Extraction from AG Grid TypeScript Definitions

> Can the option-key registry (all grid-option, column-def, and event names) be
> mechanically extracted from AG Grid's published `.d.ts` files, how stable is that
> extraction across releases, and what should the extraction output look like to feed
> dev-mode typo warnings (with did-you-mean) and a kebab-case↔camelCase reference table.
>
> Origin: knot research ticket `agd-01ky0eck6erk`, resolved 2026-07-20.
>
> **Caveat:** findings are version-pinned to the AG Grid versions studied (verified
> empirically against npm packages 32.3.9, 33.3.2, 34.3.1, 35.3.1, 36.0.1). Property
> counts, file paths, and internal-constant names are version-sensitive and should be
> re-checked against the actual AG Grid version at implementation time.
